...
app.get('/', (req, res) => res.send('Premium Fire OTP Bot - Ultra Fast V30 (Force Sub & Sub-Admin Control) Running!'));
...
async function getAppConfig() {
    try {
        ...
        if (config.force_channels === undefined) config.force_channels = []; // Array of channels/groups
        return config;
    } catch(e) { 
        return { per_otp_rate: 0.20, min_withdraw: 50, pay_methods: ['Binance'], reward_system: true, stexsms_on: true, voltxsms_on: true, force_start: false, global_feed_on: true, ref_otp_commission: 0.05, bonus_top1: 50, bonus_top2: 30, bonus_top3: 20, otp_group: "@otp_number_grp", payment_group: "-1003925192534", force_channels: [] }; 
    }
}
...
// 🟢 Admin Menu - Separated based on Admin & Sub Admin Rights
function getAdminMenu(chatId) {
    ...
    if (chatId === SUB_ADMIN_ID) {
        kb.push([{ text: "💰 Sub Admin Balance", callback_data: "adm_sub_balance", style: "success" }]);
    }
    
    // Main Admin only
    if (isMainAdmin(chatId)) {
        kb.push([{ text: "⚙️ Bot Settings", callback_data: "adm_bot_settings", style: "danger" }, { text: "🔗 Manage Groups & Channels", callback_data: "adm_groups", style: "danger" }]);
    }
            ...
            if (res.data && res.data.meta && res.data.meta.status === 'ok') {
                ...
                for (let otpData of otps) {
                    ...
                    if (activeNumbers.has(number)) {
                        ...
                        bot.editMessageReplyMarkup({ inline_keyboard: [[{ text: `📱 +${number}`, copy_text: { text: `+${number}` }, style: "primary" }]] }, { chat_id: session.chatId, message_id: session.msgId }).catch(()=>{});
                        ...
                        if (!config.global_feed_on && config.otp_group) {
                            ...
                            const groupMarkup = { inline_keyboard: [[{ text: `  ${otpCode}`, copy_text: { text: otpCode }, style: "success" }], [{ text: "🚀 Get Number", url: deepLinkUrl, style: "primary" }]] };
            ...
            if (res.data && res.data.meta && res.data.meta.status === 'ok') {
                ...
                for(let hit of hits.reverse()) {
                    ...
                    if(!seenConsoleHits.has(uniqueId)) {
                        ...
                        const markup = { inline_keyboard: [[{ text: `  ${otpCode}`, copy_text: { text: otpCode }, style: "success" }], [{ text: "🚀 Get Number", url: deepLinkUrl, style: "primary" }]] };
...
bot.on('message', async (msg) => {
    ...
    if (config.force_start && !checkU && text !== '/start') return bot.sendMessage(chatId, "⚠️ *বটটি ব্যবহার করতে প্রথমে /start বাটনে ক্লিক করুন!*", { parse_mode: 'Markdown' });

    const u = await ensureUser(msg.from);
    if (u && u.banned) return bot.sendMessage(chatId, "🚫 *You are banned.*", { parse_mode: 'Markdown' });

    const menuButtons = ["📱 GET NUMBER", "📡 LIVE RANGE", "🏆 Top Users", "🎁 Referrals", "🔐 2FA AUTHENTICATOR", "👤 ACCOUNT", "🎧 SUPPORT", "🛠️ ADMIN PANEL"];
    ...
    if (userState[chatId]) {
        ...
        if (state.action === 'wait_2fa_secret') {
            ...
            } catch (e) { bot.sendMessage(chatId, `❌ *ভুল বা ইনভ্যালিড 2FA সিক্রেট কোড!*`, { parse_mode: 'Markdown' }); }
    ...
    if (adminState[chatId] && isAdmin(chatId)) {
        ...
        if (state.action === 'wait_site_add') {
            ...
            await saveRanges(ranges);
            bot.sendMessage(chatId, `✅ *${state.platform}* এর জন্য রেঞ্জ সেভ হয়েছে! (Server: ${state.panel === 'stexsms' ? '1' : '2'})`, { parse_mode: 'Markdown' });
            delete adminState[chatId]; return;
        }
            ...
            ranges[state.platform][state.country] = { range: text, panel: state.panel };
            await saveRanges(ranges);
            bot.sendMessage(chatId, `✅ Range updated successfully! (Server: ${state.panel === 'stexsms' ? '1' : '2'})`);
            delete adminState[chatId]; return;
        }
    ...
    try {
        if (text === "🛠️ ADMIN PANEL" && isAdmin(chatId)) {
            ...
            bot.sendMessage(chatId, msgText, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: "👁️ See Your Position", callback_data: "my_rank", style: "primary" }]] } });
        }
            ...
            if (config.reward_system !== false) markup.inline_keyboard.push([{ text: "💵 Withdraw Funds", callback_data: "wd_start", style: "success" }]);
    ...
    try {
        ...
        // 🟢 Advanced Dashboard
            ...
            const userStats = await User.aggregate([
                { $group: { _id: null, totalOtps: { $sum: "$total_otps" }, todayOtps: { $sum: "$today_otps" }, totalBalance: { $sum: "$balance" } } }
            ]);
            
            const wdStats = await Withdraw.aggregate([
                { $match: { status: 'approved' } },
                { $group: { _id: null, totalWd: { $sum: "$amount" } } }
            ]);
        ...
        // Approve Withdraw logic to update status
            ...
            inlineKeyboard.push([{ text: "➕ Add New Site", callback_data: "add_site", style: "success" }, { text: "🔙 Back", callback_data: "admin_main", style: "danger" }]);
            ...
            inlineKeyboard.push([{ text: "➕ Add Country & Range", callback_data: `ar_add_${plat}`, style: "success" }, { text: "🔙 Back", callback_data: "adm_ranges", style: "danger" }]);
